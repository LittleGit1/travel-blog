...
<div x-data="carousel" class="relative max-w-full overflow-hidden rounded-xl">
    <div class="absolute top-[50%] z-10 translate-y-[-50%] left-2 right-2 flex justify-between">
        <button @click="moveSlide(-1)" class="bg-[#ffffff44] hover:bg-[#ffffffcc] transition-colors duration-100 rounded-full p-2">
    ...
    <div class="slides relative flex transition-transform duration-500">
        @foreach($images as $image)
            <img class="slide min-w-full border-box aspect-square object-cover" width="800" height="800" src="{{asset('img/posts/carousel/' . $image->public_path)}}" alt="">
        @endforeach
    </div>
</div>
